...
from .models import db, setup_db, Clients, Trabajadores, Producto, Tarjeta, Orden_de_Compra, Administracion, File
...
def create_dev(test_config=None):
    dev = Flask(__name__)
    with dev.dev_context():
        dev.config['UPLOAD_FOLDER'] = 'static/images'
        setup_db(dev, test_config['database_path'] if test_config else None)
        CORS(dev, origins='http://localhost:5432')
