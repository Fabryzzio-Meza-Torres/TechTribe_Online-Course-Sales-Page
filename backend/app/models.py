...
class Trabajadores(db.Model):
    ...
    producto_id = db.Column(db.String(36), db.ForeignKey(
...
class Tarjeta(db.Model):
    ...
    id_client = db.Column(db.String(36), db.ForeignKey(
...
class Orden_de_Compra(db.Model):
    ...
    id_product = db.Column(db.String(36), db.ForeignKey(
        'products.id'), nullable=False)
    id_cliente = db.Column(db.String(36), db.ForeignKey(
        'clients.id'), nullable=False)
    created_at = db.Column(db.DateTime(timezone=True), nullable=False)
    modified_at = db.Column(db.DateTime(timezone=True), nullable=True)
...
def crear_datos_por_defecto(dev, db):
    with dev.app_context():
        ...
        if not cursos:
            curso1 = Producto(
                "Python", "100", "Domina Python y crea aplicaciones poderosas con nuestro curso. Sintaxis elegante, fácil de leer y recursos accesibles hacen que el aprendizaje sea divertido", "Curso", "3 meses")
            curso2 = Producto(
                "C++", "100", "Únete a nuestro curso de C++ y descubre cómo crear aplicaciones de alto rendimiento con una sintaxis poderosa y eficiente. ¡Aprender es desafiante pero gratificante!", "Curso", "3 meses")
            curso3 = Producto(
                "HTML/CSS", "100", "Mejora tu programación con matemáticas. Aprende cálculo, álgebra y estadística aplicada en nuestro curso para CS", "Curso", "3 meses")
            curso4 = Producto("Matematica para CS", "100",
                              "Aprende HTML y CSS para crear sitios web impresionantes y atractivos. Únete a nuestro curso para mejorar tus habilidades de diseño web y destacar en el mercado.", "Curso", "3 meses")
            ...
            db.session.add_all(
                [curso1, curso2, curso3, curso4, asesoria1, asesoria2, asesoria3, asesoria4])
        if not trabajadores:
            curso1 = Producto.query.filter(
                Producto.name == "Python", Producto.type_product == "Curso").first()
            curso2 = Producto.query.filter(
                Producto.name == "C++", Producto.type_product == "Curso").first()
            curso3 = Producto.query.filter(
                Producto.name == "HTML/CSS", Producto.type_product == "Curso").first()
            curso4 = Producto.query.filter(
                Producto.name == "Matematica para CS", Producto.type_product == "Curso").first()
            asesoria1 = Producto.query.filter(
                Producto.name == "Python", Producto.type_product == "Asesoria").first()
            asesoria2 = Producto.query.filter(
                Producto.name == "C++", Producto.type_product == "Asesoria").first()
            asesoria3 = Producto.query.filter(
                Producto.name == "HTML/CSS", Producto.type_product == "Asesoria").first()
            asesoria4 = Producto.query.filter(
                Producto.name == "Matematica para CS", Producto.type_product == "Asesoria").first()
            ...
            profesor6 = Trabajadores(
                "Jorge", "Villavicencio", "36", asesoria3.id)
